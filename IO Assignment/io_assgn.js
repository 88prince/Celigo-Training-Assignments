...
let token_auth = 'Your own Bearer token';
...
async.series([
  function(callback){
    createRestApiExportConnection(restApiExportConnectionOptions).then(res => {
      console.log(`Rest Api Export connection created with id ${res}`);
      callback(null, res);
    })
    .catch(err => {
      console.log(err);
